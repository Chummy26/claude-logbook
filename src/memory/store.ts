...
function acquireLockFile(memDir: string): boolean {
...
function releaseLockFile(memDir: string): void {
    const lockPath = path.join(memDir, "lock");
    try {
...
export class MemoryStore {
    ...
    constructor(projectDir: string) {
        this.memDir = path.join(projectDir, ".logbook");
    ...
    acquireLock(): boolean {
        return acquireLockFile(this.memDir);
    }

    releaseLock(): void {
        releaseLockFile(this.memDir);
    }
    ...
    addMemory(
        memory: Omit<Memory, "id" | "created" | "updated" | "confidence" | "accessCount">
    ): Memory | null {
    ...
    getMemories(opts?: {
        ...
        let mems =
            opts?.active === false ? this.state.memories : this.getActiveMemories();
    ...
    decayConfidence(): void {
        ...
        for (const m of this.state.memories) {
            if (m.tags.includes("superseded") || m.tags.includes("archived"))
                continue;
            const ageDays =
                (now - new Date(m.updated).getTime()) / 86400000;
    ...
    needsConsolidation(): boolean {
        ...
        return (
            ...
            (this.state.extractionCount > 0 &&
                this.state.extractionCount % 10 === 0)
        );
    }

    applyConsolidation(results: ConsolidationResult): void {
        const now = new Date().toISOString();
        ...
        for (const merge of results.merge) {
            ...
            const firstSource = this.state.memories.find(
                (m) => m.id === merge.sources[0]
            );
        ...
        this.state.memories = this.state.memories.filter((m) => {
            if (
                m.tags.includes("archived") &&
                new Date(m.updated).getTime() < fourteenDaysAgo
            ) {
                return false;
            }
            return true;
    ...
    getMemoriesForConsolidation(): Record<
        string,
    ...
    getAllMemoryCount(): {
        ...
        const active = this.state.memories.filter(
            (m) =>
                !m.tags.includes("superseded") && !m.tags.includes("archived")
        ).length;
        const archived = this.state.memories.filter((m) =>
            m.tags.includes("archived")
        ).length;
        const superseded = this.state.memories.filter((m) =>
    ...
    generateConsciousness(): string {
        const s = this.state;
        const active = this.getActiveMemories().filter(
            (m) => m.confidence > 0.3
        );
        ...
        for (const type of TYPE_ORDER) {
            ...
            for (let i = 0; i < Math.min(mems.length, limit); i++) {
                ...
                const filteredTags = m.tags.filter(
                    (t) => t !== "superseded" && t !== "archived"
                );
                const tagStr = filteredTags.length
                    ? ` [${filteredTags.join(", ")}]`
                    : "";
            ...
            if (mems.length > limit) {
                sections.push(
                    `- _...and ${mems.length - limit} more (use memory_search to find them)_`
                );
            }
        ...
        sections.push(
            "_For deeper context, use memory_search or context_status tools._"
        );
